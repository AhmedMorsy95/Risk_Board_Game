...
public class MiniMax extends Player {
    ...
    int heuristic(Player a,int mapSz) {
    	int total=0,maxHeuristic=-2000,hasOneSoldier=0;
			for(int i=0;i<a.territories.size();i++) {
				total=0;
				if(a.soldier_of_each_territory[territories.get(i)]==1) {
					hasOneSoldier=1;
				}
    ...
    double getDiceProbability(int a,int b) {
    	///
    	if(a == 3 && b == 2) return  0.3717;
    	if(a == 3 && b == 1) return  0.6597;
    	if(a == 2 && b == 2) return  0.2276;
    	if(a == 2 && b == 1) return  0.5787;
    	if(a == 1 && b == 1) return  0.4167;
    	if(a == 1 && b == 2) return  0.2546;
    	
    	return 1.0;
    }
    Player makeChangesFirst(Player a, Player b,int from,int to,int x,int y,boolean win) { // returns effects to first player
    	if(win) {
    ...
    Player makeChangesSecond(Player a, Player b,int from,int to,int x,int y,boolean win) { // returns effects to first player
    	if(win) {
    			...
    			for(int j=0;j<neighbours.size();j++) {
    				...
    				for(int dice1 = 1 ; dice1 < Math.min(4, a.soldier_of_each_territory[a.territories.get(i)]);dice1++) {
    					  for(int dice2 = 1 ; dice2 <= Math.min(2, b.soldier_of_each_territory[b.territories.get(j)]);dice2++) {
    						         ...
    						         double tmp = mx1;
    						         mx1 =Math.max(mx1 , getMiniMax(makeChangesSecond(a, b, from, to, dice1, dice2, false) , makeChangesFirst(a, b, from, to, dice1, dice2, true), depth-1, mapSz, pathProbability * getDiceProbability(dice1, dice2),!isMax));  		  
                                     if(isMax && tmp < mx1 && depth == 5) {
                                    	 optimalFrom = from;
                                    	 optimalTo = to;
                                    	 optimalS1 = dice1;
                                    	 optimalS2 = dice2;
    						         ...
    						         mn1 =Math.min(mn1 , getMiniMax(makeChangesSecond(a, b, from, to, dice1, dice2, true) , makeChangesFirst(a, b, from, to, dice1, dice2, false),  depth-1, mapSz, pathProbability * (1.0 - getDiceProbability(dice1, dice2)),!isMax));  			  
	...
	public void startAttack(Player opponent, int mapSz) {
	     getMiniMax(this, opponent, 5, mapSz,1.0,true);
	     if(optimalFrom != -1)
